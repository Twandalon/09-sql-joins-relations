...
const app = express();
const conString = 'postgres://localhost:5432/kilavolt';// DONE: Don't forget to set your own conString
...
app.post('/articles', function(request, response) {
  client.query(
    'INSERT INTO authors(author, "authorUrl") VALUE($1, $2) ON CONFLICT DO NOTHING', // DONE: Write a SQL query to insert a new author, ON CONFLICT DO NOTHING
  ...
  function queryThree(author_id) {
    client.query(
      ...
      [request.body.author_id, request.body.title, request.body.category, request.body.publishedOn, request.body.body], // DONE: Add the data from our new article, including the author_id, as data for the SQL query.
      function(err) {
        if (err) console.error(err);
